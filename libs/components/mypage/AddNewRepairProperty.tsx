...
import { CREATE_PROPERTY, CREATE_REPAIRPROPERTY, UPDATE_PROPERTY } from '../../../apollo/user/mutation';
import { GET_PROPERTY, GET_REPAIRPROPERTY } from '../../../apollo/user/query';
...
const AddRepairProperty = ({ initialValues, ...props }: any) => {
	...
	const [repairPropertyData, setRepairPropertyData] = useState<RepairPropertyInput>(initialValues);
	const [propertyType, setPropertyType] = useState<RepairPropertyType[]>(Object.values(RepairPropertyType));
	const token = getJwtToken();
	const user = useReactiveVar(userVar);

	/** APOLLO REQUESTS **/
	const [createRepairProperty] = useMutation(CREATE_REPAIRPROPERTY);

	const {
		loading: getRepairLoading,
		data: getRepairData,
		error: getRepairError,
		refetch: getRepairRefetch,
	} = useQuery(GET_REPAIRPROPERTY, {
		fetchPolicy: 'network-only',
		variables: {
			input: {
				repairPropertyId: router.query.repairPropertyId,
			},
		},
	});
	...
	useEffect(() => {
		setRepairPropertyData({
			...repairPropertyData,
			repairPropertyType: getRepairData?.getRepairProperty?.repairPropertyType || '',
			repairPropertyAddress: getRepairData?.getRepairProperty?.repairPropertyAddress || '',
			repairPropertyDescription: getRepairData?.getRepairProperty?.repairPropertyDescription || '',
			repairPropertyImages: getRepairData?.getRepairProperty?.repairPropertyImages || [],
			constructedAt: getRepairData?.getRepairProperty?.constructedAt || undefined,
		});
	}, [getRepairLoading, getRepairData]);
	...
	const doDisabledCheck = (): boolean => {
		...
		return (
			isEmptyOrInvalid(repairPropertyType) ||
			isEmptyOrInvalid(repairPropertyAddress) ||
			isEmptyOrInvalid(repairPropertyDescription) ||
			repairPropertyImages.length === 0
		);
	};

	const insertRepairPropertyHandler = useCallback(async () => {
		try {
			const result = await createRepairProperty({
				variables: {
...
AddRepairProperty.defaultProps = {
	initialValues: {
		repairPropertyType: '',
		repairPropertyAddress: '',
		repairPropertyDescription: '',
...
export default AddRepairProperty;
